...
struct TerminalConfig {
    ...
    updated_at: String,
    #[serde(default)]
    partner_checkout_url: Option<String>,
}
...
fn terminal_save_config(
    ...
    let cfg = TerminalConfig {
        ...
        registry_recovery_key_hash,
        partner_checkout_url: if input.partner_checkout_url.is_some() {
            input
                .partner_checkout_url
                .clone()
                .filter(|v| !v.trim().is_empty())
        } else {
            existing
                .as_ref()
                .and_then(|c| c.partner_checkout_url.clone())
        },
...
fn db_list_audit_log(
    ...
    if let Some(a) = filter.as_ref().and_then(|f| f.action_kind.as_ref()) {
        params.push(a.trim().to_string());
...
fn db_delete_work_entry(entry_id: String, state: State<'_, DbState>) -> Result<(), String> {
    ...
    let conn = db.connection_mut();
    let tx = conn.transaction().map_err(|e| format!("tx begin failed: {e}"))?;

    let mut stmt = tx.prepare("SELECT * FROM work_entries WHERE entry_id = ?1").map_err(|e| e.to_string())?;
    let mut rows = stmt.query([entry_id]).map_err(|e| e.to_string())?;
    let json_val = if let Some(row) = rows.next().map_err(|e| e.to_string())? {
        serde_json::json!({
            "entry_id": row.get::<_, String>(0)?,
            "work_date": row.get::<_, String>(1)?,
            "minutes_actual": row.get::<_, i64>(2)?,
            "activity_kind": row.get::<_, String>(3)?,
            "effort_phase": row.get::<_, String>(4)?,
            "title": row.get::<_, String>(5)?,
            "notes": row.get::<_, String>(6)?,
            "subject_label": row.get::<_, String>(7)?,
            "case_id": row.get::<_, Option<String>>(8)?,
            "plan_id": row.get::<_, Option<String>>(9)?,
            "audience_note": row.get::<_, String>(10)?,
            "audience_contingent": row.get::<_, String>(11)?,
            "time_start": row.get::<_, String>(12)?,
            "time_end": row.get::<_, String>(13)?,
            "referrer": row.get::<_, String>(14)?,
            "visit_kind": row.get::<_, String>(15)?,
            "anonymous_code": row.get::<_, String>(16)?,
            "event_form": row.get::<_, String>(17)?,
            "diagnostic_kind": row.get::<_, String>(18)?,
            "co_executors_text": row.get::<_, String>(19)?,
