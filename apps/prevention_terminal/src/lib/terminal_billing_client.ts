...
export async function startRuTerminalCheckout(args: {
  terminalUserId: string;
  plan: TerminalBillingPlan;
  promoCode?: string;
  buyerEmail?: string;
}): Promise<{ ok: boolean; checkout_url?: string; message?: string; promo_applied?: boolean }> {
  // First, check promo code
  if (args.promoCode?.trim()) {
    const p = await validatePromoCode(args.promoCode);
    if (p.ok && p.message.includes("100%")) {
      return { ok: true, promo_applied: true, message: p.message };
    }
  }
  ...
  const payload: Record<string, string> = {
    ...
    plan: args.plan,
    app_id: "specialist_terminal",
  };
  if (args.buyerEmail?.trim()) payload.email = args.buyerEmail.trim();

  try {
    const res = await fetch(`${platformApiBase()}/api/terminal/billing/yookassa-checkout`, {
      method: "POST",
    ...
    return { ok: false, message: data.error || "Ошибка инициализации оплаты." };
  } catch (e) {
