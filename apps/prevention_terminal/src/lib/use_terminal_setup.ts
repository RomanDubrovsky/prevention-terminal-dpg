...
export interface TerminalSetupSaveOverrides {
  displayName?: string;
...
export function useTerminalSetup() {
  ...
  const validateAll = useCallback((overrides?: TerminalSetupSaveOverrides): string | null => {
    const effOrgType = overrides?.orgType ?? orgType;
    const effRole = overrides?.role ?? profileRole;
    const effDisplayName = (overrides?.displayName ?? displayName).trim();
    const effOrgLabel = (overrides?.organizationLabel ?? installationDraft.organization_label).trim();

    if (!isEducatorLite) {
      if (effOrgType !== "education" && effOrgType !== "commercial") {
        return "Выберите тип организации.";
      }
      if (!profileRolesForOrgSegment(effOrgType).includes(effRole)) {
        return "Выберите вашу роль.";
      }
    }
    if (!effDisplayName) return "Укажите имя пользователя.";
    if (!effOrgLabel) return "Укажите название организации.";

    const effInstallation: InstallationMetaInput = {
      ...installationDraft,
      organization_label: effOrgLabel,
      organization_type: effOrgType === "commercial" ? "commercial_center" : (installationDraft.organization_type || "school"),
    };

    const installationValidation = validateInstallationDraft(effInstallation, {
      requireSettlement: Boolean(effInstallation.settlement.trim()),
    });
    if (installationValidation) return installationValidation;

    const effOrgDraft: OrgProfileInput = {
      ...orgDraft,
      display_name: effOrgLabel,
      org_kind: orgDraft.org_kind || (effOrgType === "commercial" ? "commercial_center" : "combined_school"),
    };

    const orgValidation = validateOrgProfileDraft(effOrgDraft, {
    ...
    const effPreset = resolveWorkspacePreset(mode, effOrgType || null, effRole === "psychologist" ? "specialist" : null);

    if (effPreset === "specialist") {
      const payload: SpecialistProfileInput = {
        ...buildSpecialistPayload(),
        display_name: effDisplayName,
        role_text: jobTitle || specialistDraft.role_text || "Психолог",
      };
      const specialistValidation = validateSpecialistProfileDraft(payload);
      if (specialistValidation) return specialistValidation;
    } else if (!isEducatorLite && !(jobTitle || specialistDraft.role_text).trim()) {
      return "Укажите должность.";
    }
    const federationError = validateFederationLinks({
      workspacePreset: effPreset,
      isManagerPreset,
      territorialManager,
      parentIn,
      childIn,
    ...
    profileRole,
    profileValidationOptions,
    specialistDraft.role_text,
    territorialManager,
  ]);

  const save = useCallback(async (overrides?: TerminalSetupSaveOverrides) => {
    const effOrgType: OrgTypePreset = (overrides?.orgType ?? orgType) || "education";
    const effRole: OnboardingProfileRole = overrides?.role ?? profileRole ?? "psychologist";
    const effDisplayName = (overrides?.displayName ?? displayName).trim() || "Специалист";
    const defaultOrg = effOrgType === "commercial" ? "Частная практика" : "Психологическая служба";
    const effOrgLabel = (overrides?.organizationLabel ?? installationDraft.organization_label).trim() || defaultOrg;

    const validationError = validateAll(overrides ? {
    ...
    const effMode: TerminalMode = effRole === "psychologist" ? "specialist" : mode;
    const effPreset = resolveWorkspacePreset(effMode, effOrgType || null, effRole === "psychologist" ? "specialist" : null);
    const effManagerPreset = effPreset === "manager";
    const effModules = modules && Object.keys(modules).length > 0
      ? modules
      : defaultEnabledModules(effMode, effOrgType, effPreset);

    const effEducationOrgType = overrides?.educationOrgType ?? orgDraft.education_org_type ?? "general_school";
    const effOrgSphere = overrides?.orgSphere ?? orgDraft.org_sphere ?? (effOrgType === "commercial" ? "other" : "education_system");
    const effInstallationOrgType = overrides?.organizationType ?? (
      effOrgType === "commercial"
        ? (installationDraft.organization_type || "private_practice")
        : (installationDraft.organization_type || "school")
    );

    const effInstallation: InstallationMetaInput = {
      ...installationDraft,
      organization_label: effOrgLabel,
      organization_type: effInstallationOrgType,
      settlement: installationDraft.settlement.trim() || "Онлайн",
    };

    const effOrgDraft: OrgProfileInput = {
      ...orgDraft,
      display_name: effOrgLabel,
      org_sphere: effOrgSphere,
      education_org_type: effOrgSphere === "education_system" ? effEducationOrgType : null,
      org_kind: orgDraft.org_kind || (effOrgType === "commercial" ? "private_practice" : "combined_school"),
    };

    const effSpecialistPayload: SpecialistProfileInput = {
      ...buildSpecialistPayload(),
      display_name: effDisplayName,
      role_text: jobTitle || specialistDraft.role_text || (effRole === "psychologist" ? "Психолог" : "Руководитель"),
    };

    try {
      return await persistTerminalSetup({
        installationInput: effInstallation,
        orgDraft: effOrgDraft,
        specialistPayload: effSpecialistPayload,
        edition,
        mode: effMode,
        workspacePreset: effPreset,
        orgType: effOrgType,
        managerScopeChoice,
        jobTitle: jobTitle || effSpecialistPayload.role_text,
        childCode,
        parentCode,
        parentIn,
        childIn,
        modules: effModules,
        registryEnabled,
        isManagerPreset: effManagerPreset,
        centerId,
        setupToken,
      });
    } finally {
