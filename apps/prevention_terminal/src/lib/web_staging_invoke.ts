...
function defaultSitePortal(store?: StagingStore) {
  ...
  return {
    center_id: slugifyCenterId(orgLabel || "staging-center"),
    setup_token: "staging-setup-token-32chars-min",
...
export async function invoke<T>(cmd: string, args: Record<string, unknown> = {}): Promise<T> {
  switch (cmd) {
    ...
    case "site_portal_get":
      return (readStagingStore().sitePortal || defaultSitePortal(readStagingStore())) as T;

    case "site_portal_ensure": {
      const orgName = String(args.organizationName || "Организация");
      ...
      return withStore((store) => {
        ...
        const cfg = {
          center_id: cid,
          setup_token: String(existing.setup_token || "staging-setup-token-32chars-min"),
    ...
    case "site_portal_update":
      return withStore((store) => {
        ...
        const next = {
          center_id: String(existing.center_id || "staging-center"),
          setup_token: String(existing.setup_token || "staging-setup-token-32chars-min"),
          inbox_login:
