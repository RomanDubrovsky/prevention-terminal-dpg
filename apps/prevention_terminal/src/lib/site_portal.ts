...
export async function ensureSitePortal(organizationName: string): Promise<SitePortalConfig> {
  const centerId = slugifyCenterId(organizationName.trim() || "center");
  return invoke<SitePortalConfig>("site_portal_ensure", {
    organizationName: organizationName.trim() || "Организация",
    centerId,
  });
}
