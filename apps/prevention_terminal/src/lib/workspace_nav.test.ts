...
describe("workspace nav", () => {
  it("dashboard, calendar, AI helper, registry, then purple cluster workflow tools, reports & system", () => {
    const nav = buildSpecialistNav(baseCfg());
    assert.deepEqual(
      nav.slice(0, 5).map((i) => i.id),
      [
        "dashboard",
        "inbox",
        "calendar",
    ...
    const registryItem = nav.find((i) => i.id === "registry");
    assert.equal(registryItem?.label, "Реестр");
  ...
  it("hides school-only modules for commercial orgs even when enabled_modules empty", () => {
    ...
    assert.equal(nav.find((i) => i.id === "registry")?.label, "Реестр");
  });
  ...
  it("manager nav hides AI assistant by default or when modules disabled", async () => {
    const { buildManagerNav } = await import("./workspace_nav.ts");
    const managerNavDefault = buildManagerNav(
      { aiSubscriptionActive: true },
      { ...baseCfg(), mode: "manager", workspace_preset: "manager", enabled_modules: { ai_assistant: false } },
    );
    assert.deepEqual(managerNavDefault.map((i) => i.id), ["dashboard", "analytical_report", "leads", "users", "specialists", "feedback", "settings"]);

    const managerNavWithAi = buildManagerNav(
      { aiSubscriptionActive: true },
      { ...baseCfg(), mode: "manager", workspace_preset: "manager", enabled_modules: { ai_assistant: true } },
    );
    assert.deepEqual(managerNavWithAi.map((i) => i.id), ["dashboard", "analytical_report", "leads", "users", "specialists", "ai_consultant", "feedback", "settings"]);
  });
});
