...
export function useTerminalSubscription(terminalUserId?: string) {
  ...
  const reload = useCallback(async () => {
    if (import.meta.env.VITE_FORCE_PREMIUM === "true" || (isWebStaging() && readStagingAiPreview()) || terminalUserId?.startsWith("tu-demo-")) {
      setSub({
        edition: getTerminalEdition(),
        active: true,
        paywall_url: defaultPaywallUrl(),
        message: terminalUserId?.startsWith("tu-demo-") ? "Демо-режим с активной подпиской ИИ." : (import.meta.env.VITE_FORCE_PREMIUM === "true" ? "Демо-режим с активной подпиской ИИ." : "Staging: превью экрана с активной подпиской ИИ."),
        features: {
          expert: true,
          architect: true,
          supervisor_bot: true,
