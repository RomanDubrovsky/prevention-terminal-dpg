...
export async function exportAnnualReportToDocx(
  schoolYear: string,
  specialistName: string,
  sections: { title: string; text: string }[],
  formTitle: string = "Аналитический отчет за год (Форма 5)"
): Promise<ArrayBuffer> {
  const children = [
    title(formTitle),
    metaLine("Учебный год", schoolYear),
    metaLine("Педагог-психолог", specialistName),
    metaLine("Дата формирования", new Date().toLocaleDateString("ru-RU")),
    space(),
  ];

  for (let i = 0; i < sections.length; i++) {
    children.push(sectionHeading(`${i + 1}. ${sections[i].title}`));
    const paragraphs = sections[i].text.split("\n").filter((p) => p.trim());
    for (const p of paragraphs) {
      children.push(paragraph(p.trim()));
    }
  }

  const doc = new Document({
    creator: "Prevention AI",
