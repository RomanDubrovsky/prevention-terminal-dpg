...
export type WorkloadJournalTab = "all" | "recycle_bin" | SchoolFormId;
...
export async function listDeletedWorkEntries(): Promise<WorkEntry[]> {
  const auditLogs = await invoke<any[]>("db_list_audit_log", {
    filter: {
  ...
  return auditLogs.map((log) => {
    try {
      const parsed = typeof log.changed_fields === "string" 
...
export async function restoreWorkEntry(entry: WorkEntry): Promise<void> {
