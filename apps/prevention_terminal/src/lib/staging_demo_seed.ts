...
export type DemoWorkspace = "specialist" | "manager" | "school" | "authority" | "educator" | "school_manager";
...
function readDemoParam(): DemoWorkspace | null {
  ...
  const rawRole = search.get("role")?.trim().toLowerCase();
  const raw = search.get("demo");
  const v = (raw || "").trim().toLowerCase();
  
  if (v === "school") {
    if (rawRole === "educator" || rawRole === "pedagogue" || rawRole === "teacher") return "educator";
    if (rawRole === "manager" || rawRole === "director") return "school_manager";
    return "school";
  }

  if (rawRole === "educator" || rawRole === "pedagogue" || rawRole === "teacher") return "educator";
  if (rawRole === "manager" || rawRole === "director") return "manager";
  
  if (raw == null && rawRole == null) return null;
  
  const host = window.location.hostname;
  const isIdaHost = host.includes("ida-ai.chat") || host.includes("ida.chat");
  if (isIdaHost) {
    if (v === "manager" || rawRole === "manager") return "manager";
    return "specialist";
  }
  
  if (v === "educator") return "educator";
  if (v === "manager") return "manager";
  if (v === "authority") return "authority";
  if (v === "school_manager") return "school_manager";
...
export function readDemoWorkspace(): DemoWorkspace {
  ...
  try {
    ...
    if (stored === "educator") return "educator";
    if (stored === "school_manager") return "school_manager";
  } catch {
    /* ignore */
  }
  return "specialist";
...
function markDemoActive(workspace: DemoWorkspace): void {
  ...
  const flag = workspace === "manager" ? "manager"
    ...
    : workspace === "educator" ? "educator"
    : workspace === "school_manager" ? "school_manager"
    : "1";
...
export function ensureDemoSeed(): boolean {
  ...
  if (workspace === "school" || workspace === "authority" || workspace === "educator" || workspace === "school_manager" || (isSchoolHost && workspace === "manager")) {
...
export function reseedDemo(workspace?: DemoWorkspace): void {
  ...
  if (ws === "school" || ws === "authority" || ws === "educator" || ws === "school_manager" || (isSchoolHost && ws === "manager")) {
