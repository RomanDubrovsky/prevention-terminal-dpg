...
import { clearDemoFlags } from "./staging_demo_seed.ts";
...
import { updateSitePortal } from "./site_portal.ts";
...
export async function persistTerminalSetup(args: {
  ...
  if (args.centerId || args.setupToken) {
  ...
  const userEmail = localStorage.getItem("platform_email") || terminalConfig.contact_email || undefined;
