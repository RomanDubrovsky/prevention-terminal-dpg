...
export function useAiProfessorVoice(onSpeakStateChange?: (speaking: boolean) => void) {
  ...
  const speak = useCallback(
    async (text: string, lang = getTerminalEdition() === "intl" ? "en-US" : "ru-RU") => {
      ...
      // Instantiate Audio synchronously in the user gesture click handler frame
      const audio = new Audio();
      audioRef.current = audio;
      currentModeRef.current = "server";

      // Try server-side Author / Neural Voice clone
      try {
        setIsSynthesizing(true);
        const url = `${platformApiBase()}/api/tts/voice-clone?text=${encodeURIComponent(cleanText.slice(0, 1500))}&lang=${lang.split("-")[0]}`;
        ...
        setIsSynthesizing(false);
        console.warn("Server TTS voice clone error, native fallback disabled:", err);
        // Do not fall back to native speech
      }
    },
    [isSupported, isMuted, stop, onSpeakStateChange, playNativeSpeech]
  );
