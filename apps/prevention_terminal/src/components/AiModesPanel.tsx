...
export default function AiModesPanel(props: AiModesPanelProps) {
  ...
  return (
    <section className={`ai-modes-panel ${inline ? "" : "card"}`}>
      {!hideHeader && <h2>{panelTitle}</h2>}

      {!hideHint && panelHint && <p className="muted ai-mode-hint">{panelHint}</p>}
      ...
      {(lastUserQuestion || lastResult?.reply) && (
        <div className="ai-chat-thread">
          {lastUserQuestion && (
            <div className="ai-user-prompt">
              <div className="ai-user-prompt-header">💬 {t("Ваш запрос:", "Your question:")}</div>
              <div className="ai-user-prompt-body">{lastUserQuestion}</div>
            </div>
          )}
          {lastResult?.reply && (
            <div className="ai-reply">
              <div className="ai-reply-header">🤖 {t("Ответ ИИ:", "AI Answer:")}</div>
              <FormattedMarkdown content={lastResult.reply} />
            </div>
          )}
        </div>
      )}
...
export function FormattedMarkdown({ content }: { content: string }) {
  ...
  return (
    <div className="ai-reply-formatted">
      {blocks.map((block, idx) => {
        ...
        if (isList) {
          return (
            <ul key={idx} className="ai-reply-list">
              {lines.map((line, lIdx) => (
                <li key={lIdx}>
                  {formatInlineMarkdown(line.replace(/^\s*[\*•\-]\s+/, "").replace(/^\s*\d+[\.\)]\s+/, ""))}
                </li>
              ))}
            </ul>
          );
        ...
        return (
          <p key={idx} className="ai-reply-p">
            {lines.map((line, lIdx) => (
              <span key={lIdx}>
                {lIdx > 0 && <br />}
