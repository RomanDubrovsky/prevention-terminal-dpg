...
export default function IdaAuthGate({ onAuthorized }: IdaAuthGateProps) {
  ...
  const landingUrl = isRu ? "https://ida-ai.chat/ru/" : "https://ida-ai.chat/";
...
const SUPABASE_PSY_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVjcHBtY3NjZWdsbXFzb2dpandzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzExMzc0NjMsImV4cCI6MjA4NjcxMzQ2M30.ZFUtTkV3fw10CICsXG1LMvLxvu4E9W5q_yWzpeKOOlE";
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      let res = await fetch("https://auth.prevention.school/auth/otp/send", {
        ...
        body: JSON.stringify({ email, app_id: "ida_web_terminal", website: honeypot }),
      }).catch(() => null);

      // Only fallback to Supabase if network connection to auth.prevention.school completely failed (res === null)
      if (res === null) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Direct Supabase fallback if auth.prevention.school network was unreachable
