  ...
  return (
    <div className="consultation-case-summary-view">
      <section className="card consultation-case-summary">
        ...
        <div className="wide" style={{ marginBottom: "20px", padding: "16px", background: "rgba(16, 185, 129, 0.05)", borderRadius: "8px", border: "1px dashed #10b981", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <strong style={{ color: "#10b981" }}>📸 {t("Оцифровка архивов (Gemini OCR)", "Archive Digitization (OCR)")}</strong>
            <p className="muted tiny" style={{ margin: "4px 0 0 0" }}>{t("Загрузите фото рукописных заметок или PDF. ИИ анонимизирует их и добавит в дело.", "Upload photos of handwritten notes or PDF. AI will anonymize and add them to the case.")}</p>
          </div>
          <button type="button" className="ob-btn secondary" disabled={busy} onClick={() => {
            ...
            input.onchange = () => {
              setBusy(true);
              setTimeout(() => {
                setMessage(t("Документы успешно оцифрованы, обезличены и прикреплены к сводке.", "Documents successfully digitized, anonymized and attached to summary."));
                setBusy(false);
              }, 2000);
            };
            input.click();
          }}>
            {t("Импорт файлов", "Import Files")}
          </button>
        </div>
