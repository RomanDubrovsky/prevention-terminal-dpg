    ...
    async (plan: TerminalBillingPlan) => {
      ...
      try {
        ...
        if (editionRu && !isForeignPayment) {
          ...
          setStatus(
            "Страница оплаты Lava.top открыта в новой вкладке. После оплаты нажмите «Проверить активацию».",
          );
          return;
        }
      ...
      {editionRu && !partnerCheckoutUrl && (
        <div className="ai-subscription-region-selector" style={{ marginBottom: "16px", padding: "10px", borderRadius: "8px", border: "1px solid var(--border-strong)", background: "var(--bg-surface-lowest)" }}>
          ...
          <p className="muted tiny" style={{ marginTop: "4px", marginInlineStart: "22px" }}>
            По умолчанию используется Lava.top для карт РФ. Если вы находитесь вне России, выберите этот пункт для оплаты через PayPal.
          </p>
        </div>
      )}
        ...
        <article className="ai-pricing-tier ai-pricing-tier-pro">
          ...
          <div className="ai-pricing-pay-row">
            ...
            >
              ...
              <span className="ai-pricing-pay-price">{isForeignPayment ? "$9" : monthPrice}</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span className="ai-pricing-pay-price">{isForeignPayment ? "$79" : yearPrice}</span>
            </button>
          </div>
        </article>
      </div>
