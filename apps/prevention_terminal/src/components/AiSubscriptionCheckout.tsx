...
export default function AiSubscriptionCheckout(props: AiSubscriptionCheckoutProps) {
  ...
  useEffect(() => {
    import("@tauri-apps/api/core").then(({ invoke }) => {
      invoke<any>("terminal_get_config").then((cfg) => {
        if (cfg?.partner_checkout_url) {
          setPartnerCheckoutUrl(cfg.partner_checkout_url);
        }
      }).catch(() => {});
    });
  ...
  const openCheckout = useCallback(
    async (plan: TerminalBillingPlan) => {
      ...
      try {
        if (partnerCheckoutUrl) {
          const url = new URL(partnerCheckoutUrl);
          url.searchParams.set("terminal_user_id", terminalUserId);
        ...
        if (editionRu && !isForeignPayment) {
          const result = await startRuTerminalCheckout({
            terminalUserId,
            plan,
            promoCode: promo,
            buyerEmail,
          ...
          if (result.checkout_url) {
            ...
            setStatus("Страница оплаты ЮKassa открыта. После завершения проверьте активацию.");
            return;
          }
        ...
        if (result.checkout_url) {
          window.open(result.checkout_url, "_blank", "noopener,noreferrer");
          setStatus("Страница оплаты PayPal открыта. После завершения проверьте активацию.");
          return;
        }
        setStatus(result.message || "Не удалось начать оплату.");
      } finally {
    ...
    [buyerEmail, editionRu, isForeignPayment, onActivated, promo, terminalUserId],
  );
  ...
  return (
    ...
    >
      ...
      {editionRu && !partnerCheckoutUrl && (
        <div className="ai-subscription-region-selector" style={{ marginBottom: "16px", padding: "10px", borderRadius: "8px", border: "1px solid var(--border-strong)", background: "var(--bg-surface-lowest)" }}>
          <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "13px" }}>
            ...
            />
            <span>Оплатить из-за рубежа (PayPal)</span>
          </label>
          <p className="muted tiny" style={{ marginTop: "4px", marginInlineStart: "22px" }}>
            По умолчанию используется ЮKassa для карт РФ. Если вы находитесь вне России, выберите этот пункт для оплаты через PayPal.
          </p>
        </div>
      )}
      ...
      <div className="ai-pricing-grid">
        ...
        <article className="ai-pricing-tier ai-pricing-tier-pro">
          ...
          <div className="ai-pricing-pay-row" style={{ flexDirection: "column", gap: "8px" }}>
            <a
              href="mailto:admin@prevention-ai.ru?subject=Заявка на подключение ИИ&body=Здравствуйте! Прошу активировать доступ к ИИ для терминала ID: "
              className="ai-pricing-pay-primary"
              style={{ textAlign: "center", textDecoration: "none", display: "block" }}
            >
              <span>Подать заявку (admin@prevention-ai.ru)</span>
            </a>
            <a
              href="https://t.me/RomanDubrovsky"
              target="_blank"
              rel="noreferrer"
              className="ai-pricing-pay-outline"
              style={{ textAlign: "center", textDecoration: "none", display: "block" }}
            >
              <span>Написать в Telegram (@RomanDubrovsky)</span>
            </a>
          </div>
        </article>
      </div>
      ...
      <p className="muted tiny ai-subscription-legal">
        {isForeignPayment
          ? "Оплата в долларах через международный процессинг (PayPal). Без автоматических списаний."
          : AI_SUBSCRIPTION_LEGAL.checkoutNote}
      </p>
      ...
      <p className="muted tiny ai-subscription-legal">
        {isForeignPayment ? "Оператор витрины и оплаты — Stripe / PayPal Inc." : AI_SUBSCRIPTION_LEGAL.operator}
      </p>
    </section>
  );
}
