...
import {
  ...
  SELECTABLE_EDUCATION_ORG_TYPES,
  YOUTH_ORG_TYPE_VALUES,
  YOUTH_ORG_TYPE_LABEL,
  ORG_SPHERE_LABEL,
  ORG_SPHERE_VALUES,
} from "../../lib/terminal_profiles.ts";
import { educationOrgTypeToLegacy, type EducationOrgType, type OrgSphere, type YouthOrgType } from "../../lib/org_sphere.ts";
import {
        ...
        {orgType !== "commercial" && (
          <>
            ...
            {orgDraft.org_sphere === "education_system" && (
              <label className="field">
                <span>{t("Тип образовательной организации", "Educational Organization Type")}</span>
                <select
                  value={orgDraft.education_org_type ?? "general_school"}
                  onChange={(e) => {
                  ...
                  disabled={busy}
                >
                  {SELECTABLE_EDUCATION_ORG_TYPES.map((type) => (
            ...
            {orgDraft.org_sphere === "youth_policy" && (
              <label className="field">
                <span>{t("Тип организации молодёжной политики", "Youth Organization Type")}</span>
                <select
                  value={orgDraft.org_sphere_other || "youth_municipal_center"}
                  onChange={(e) => {
                  ...
                  disabled={busy}
                >
                  {YOUTH_ORG_TYPE_VALUES.map((type) => (
                    <option key={type} value={type}>
                      {YOUTH_ORG_TYPE_LABEL[type]}
                    </option>
                  ))}
                </select>
              </label>
