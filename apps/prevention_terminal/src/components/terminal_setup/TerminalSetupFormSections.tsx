      ...
      <>
        ...
        {(orgType === "education" || orgType === "commercial") &&
          profileRolesForOrgSegment(orgType).map((role) => {
            ...
            return (
              <label key={role} className="ob-card" style={roleLocked && profileRole !== role ? { opacity: 0.45, cursor: "not-allowed" } : undefined}>
                <input
                  type="radio"
                  name="profileRole"
                  checked={profileRole === role}
                  onChange={() => applyProfileRole(role)}
                  disabled={busy || isEducatorLite || roleLocked}
                />
      ...
      <>
        ...
        <div className="ob-section-body ob-section-body--full">
          <FederationStep
            ...
            onCenterIdChange={setup.setCenterId}
