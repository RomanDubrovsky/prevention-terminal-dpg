...
export default function TerminalSetupFormSections(props: TerminalSetupFormSectionsProps) {
      ...
      <>
        ...
        {ONBOARDING_ORG_SEGMENT_OPTIONS.map((opt) => (
          <label key={opt} className="ob-card">
            ...
            />
            <strong>{ONBOARDING_ORG_SEGMENT[opt].title}</strong>
            <span>{ONBOARDING_ORG_SEGMENT[opt].hint}</span>
          </label>
        ))}
        </div>
      </>
  ...
  if (section === "profile") {
    return (
      <>
        ...
        <div className="ob-choices ob-section-body" style={isSettings ? { opacity: 0.65 } : undefined}>
        {(orgType === "education" || orgType === "commercial") &&
          profileRolesForOrgSegment(orgType).map((role) => {
            ...
            return (
              <label key={role} className="ob-card">
                <input
                  type="radio"
                  name="profileRole"
                  checked={profileRole === role}
                  onChange={() => applyProfileRole(role)}
                  disabled={busy || isEducatorLite || isSettings}
                />
  ...
  if (section === "modules" && workspacePreset === "specialist") {
    return (
      <>
        {breadcrumbs}
        <div className="ob-modules ob-section-body">
        <ModulesStepIntro />
        {SPECIALIST_PRODUCT_BUNDLES.filter((bundle) => !bundle.schoolLikeOnly || schoolLike).map((bundle) => (
          <div key={bundle.id} className="ob-mod">
            <label>
              ...
              />
              <span>
                <strong>{bundle.title_ru}</strong>
              </span>
            </label>
            <p className="ob-mod-desc">{bundle.description_ru}</p>
