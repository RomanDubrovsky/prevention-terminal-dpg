  ...
  return (
    <div className="ipr-contact-sed-block" style={{ marginTop: "24px" }}>
      ...
      <div className="visits-badge-strip" style={{ marginBottom: "16px", display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
        <span className="muted tiny" style={{ marginRight: "6px" }}>Связанные следы дела:</span>
        {caseVisits.length === 0 ? (
          <span className="muted tiny">Консультации отсутствуют.</span>
        ) : (
          caseVisits.map((v: any, idx: number) => (
            ...
            >
              📝 Сессия {caseVisits.length - idx} ({v.visit_date || new Date(v.created_at).toLocaleDateString("en-US")})
            </span>
          ))
        )}

        {/* Найденные экспертные протоколы */}
      ...
      <div className="group-session-editor-tabs" style={{ display: "flex", gap: "10px", marginBottom: "20px", borderBottom: "1px solid var(--border)", paddingBottom: "10px" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "8px" }}>
          <span className="muted tiny">Кумулятивный индекс риска:</span>
          <strong
            style={{
              padding: "4px 10px",
              borderRadius: "20px",
          ...
          >
            {riskStats.totalCount} {riskStats.totalCount > 10 ? "🔴 Высокий" : riskStats.totalCount > 4 ? "🟡 Средний" : "🟢 Низкий"}
          </strong>
        </div>
      </div>

      {/* ── Вкладка 1: Личное дело ────────────────────────────────────────── */}
      {activeTab === "profile" && (
        <div className="contact-tab-content">
          <details open={openSections.demographics} className="consultation-form-section wide" style={{ marginBottom: "16px" }}>
            <summary onClick={(e) => { e.preventDefault(); toggleSection("demographics"); }}>
              1.1. Социально-демографические данные
            </summary>
