    ...
    <Wrapper className={wrapperClass}>
      ...
      <form className="dap-note-form intake-grid" onSubmit={handleSubmit}>
        ...
        {!cardSaved && entries.length > 0 && (
          <div className="wide" style={{ marginBottom: "20px", padding: "16px", background: "rgba(139, 92, 246, 0.04)", borderRadius: "8px", border: "1px dashed var(--violet)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <strong style={{ color: "var(--violet)" }}>🧠 {t("Подготовка к сессии (Брифинг)", "Session Prep Briefing")}</strong>
                <p className="muted tiny" style={{ margin: "4px 0 0 0" }}>{t("ИИ составит 5 буллитов (на чем остановились, что спросить) на основе прошлых встреч.", "AI will create 5 bullets (where you left off, what to ask) based on previous meetings.")}</p>
              </div>
              <button type="button" className="ob-btn secondary" disabled={prepBriefingBusy} onClick={handleGenerateBriefing}>
                {prepBriefingBusy ? "Генерация..." : t("Сгенерировать брифинг", "Generate Briefing")}
              </button>
            </div>
            {prepBriefing && (
              <div style={{ marginTop: "12px", padding: "12px", background: "#fff", borderRadius: "6px", border: "1px solid var(--line)", whiteSpace: "pre-wrap" }}>
                {prepBriefing}
              </div>
            )}
          </div>
