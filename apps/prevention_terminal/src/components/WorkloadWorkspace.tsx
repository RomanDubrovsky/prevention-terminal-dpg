...
import { listRegistrySubjects, type RegistrySubjectSummary } from "../lib/registry_store.ts";
...
export default function WorkloadWorkspace(props: WorkloadWorkspaceProps) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === "recycle_bin") {
        rows = await listDeletedWorkEntries();
      } else {
        rows = await listWorkEntries({
  ...
  }, [activeTab, activityFilter, month.from, month.to]);
  ...
  async function openGroupNew() {
    ...
    if (cfg?.registry_enabled && registrySubjects.length === 0) {
      try {
        const subjects = await listRegistrySubjects();
        setRegistrySubjects(subjects);
      } catch (e) {
        setSaveErr(e instanceof Error ? e.message : String(e));
      }
    }
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (draft.time_start && draft.time_end) {
      if (draft.time_start >= draft.time_end) {
        setSaveErr("Время начала должно быть раньше времени окончания.");
        return;
      }
      const hasOverlap = entries.some((entry) => {
        if (entry.entry_id === savedEntryId) return false;
        if (entry.work_date !== draft.work_date) return false;
        if (!entry.time_start || !entry.time_end) return false;
        return draft.time_start < entry.time_end && draft.time_end > entry.time_start;
      });
      if (hasOverlap) {
        setSaveErr("Обнаружена накладка в расписании (пересечение с другой записью).");
        return;
      }
    }
  ...
  async function onGroupSubmit(e: FormEvent) {
    e.preventDefault();
    setSaveErr(null);
    setSaveOk(null);
    if (!draft.work_date.trim()) {
    ...
    if (selectedSubjectIds.size === 0) {
      setSaveErr("Выберите хотя бы одного участника из списка.");
      return;
    }
    try {
      for (const caseId of selectedSubjectIds) {
        const subject = registrySubjects.find(s => s.case_id === caseId);
        if (!subject) continue;
        const entry = { ...draft };
        entry.entry_id = newWorkEntryId();
        entry.case_id = caseId;
        // set subject_label to full name + grade
        const parts = [subject.profile.full_name];
        if (subject.profile.grade_class) parts.push(subject.profile.grade_class);
        entry.subject_label = parts.join(", ");
        await addWorkEntry(entry);
      }
      setSaveOk(`Групповая запись добавлена для ${selectedSubjectIds.size} участников.`);
      setSelectedSubjectIds(new Set());
      setGroupEditorOpen(false);
      await reload();
    } catch (err) {
  ...
  async function onRestore() {
    ...
    try {
      ...
      const entryToRestore = entries.find(e => e.entry_id === savedEntryId);
      if (!entryToRestore) throw new Error("Entry not found in state");
      await restoreWorkEntry(entryToRestore);
      setSaveOk("Запись восстановлена.");
      closeEditor();
      await reload();
    } catch (err) {
      setSaveErr(err instanceof Error ? err.message : String(err));
  ...
  return (
    <div className="workspace-panel-stack workload-journal">
            ...
            {activeTab !== "recycle_bin" && (
              <>
                ...
                {cfg?.registry_enabled && (
                  <button type="button" className="ob-btn secondary" onClick={() => void openGroupNew()}>
                    Групповая (списком)
                  </button>
                )}
              </>
        ...
        <section className="card workload-editor">
          <div className="workload-editor-head">
            ...
            <div className="workload-editor-actions">
              {savedEntryId && activeTab === "recycle_bin" && (
                <button type="button" className="ob-btn success" onClick={() => void onRestore()}>
                  Восстановить
                </button>
              )}
              {savedEntryId && activeTab !== "recycle_bin" && (
      ...
      {groupEditorOpen && (
        <section className="card workload-editor">
          <div className="workload-editor-head">
            <h3>Групповое добавление (по списку)</h3>
            <div className="workload-editor-actions">
              <button type="button" className="ob-btn secondary" onClick={closeEditor}>
                Закрыть
              </button>
          ...
          <form className="workload-form" onSubmit={(e) => void onGroupSubmit(e)}>
            <div className="workload-form-grid">
              <label>
                Дата
                <input
              ...
              <label>
                ...
                <input
                  ...
                  value={draft.title}
                  onChange={(e) => setDraft((d) => ({ ...d, title: e.target.value }))}
                  placeholder="Тема группового занятия или диагностики"
                />
              </label>
            </div>

            <div className="group-add-subject-picker" style={{ margin: "1rem 0", padding: "1rem", border: "1px solid var(--border-color)", borderRadius: "6px" }}>
              <h4 style={{ margin: "0 0 1rem 0" }}>Выберите участников ({selectedSubjectIds.size}):</h4>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "0.5rem", maxHeight: "300px", overflowY: "auto" }}>
                {registrySubjects.map(sub => (
                  <label key={sub.case_id} style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", fontSize: "0.9rem" }}>
                    <input
                      type="checkbox"
                      checked={selectedSubjectIds.has(sub.case_id)}
                      onChange={(e) => {
                        const newSet = new Set(selectedSubjectIds);
                        if (e.target.checked) newSet.add(sub.case_id);
                        else newSet.delete(sub.case_id);
                        setSelectedSubjectIds(newSet);
                      }}
                    />
                    {sub.profile.full_name || "Без имени"} {sub.profile.grade_class ? `(${sub.profile.grade_class})` : ""}
                  </label>
                ))}
                {registrySubjects.length === 0 && <p className="muted tiny">В реестре нет записей.</p>}
              </div>
            </div>
            ...
            <div className="workload-form-actions">
              <button type="submit" className="ob-btn success">
                Добавить {selectedSubjectIds.size} записей
              </button>
            </div>
          </form>
        </section>
