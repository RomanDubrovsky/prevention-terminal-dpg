...
function UpdateCheckMessage({ result }: { result: UpdateCheckResult }) {
  ...
  if (result.status === "up-to-date") {
    return (
      <div className="update-result update-result-uptodate">
        <p className="ok">{t("Установлена актуальная версия.", "The latest version is installed.")}</p>
        {result.latest && (
          <div className="update-links" style={{ marginTop: '8px' }}>
            <a href={result.latest.download_url} target="_blank" rel="noreferrer">
              {t(`Скачать установщик для Windows (v${result.latest.version})`, `Download Windows installer (v${result.latest.version})`)}
            </a>
          </div>
        )}
      </div>
