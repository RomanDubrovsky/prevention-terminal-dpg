...
export default function IprWorkspacePanel(props: IprWorkspacePanelProps) {
  ...
  async function generateIprFromConsultations() {
    if (!registrySubjectId) {
      setError("Сначала выберите человека из реестра.");
      return;
    }
    if (!confirm("Сгенерировать программу сопровождения на основе истории консультаций?")) return;
    setBusy(true);
    setError(null);
    try {
      if (import.meta.env.VITE_TERMINAL_STAGING === "true") {
        const { readStagingStore, writeStagingStore } = await import("../lib/web_staging.ts");
        const store = readStagingStore();
        store.iprs.push({
            id: `ipr-${Date.now()}`,
            case_id: registrySubjectId,
            title: "Демо: Программа сопровождения",
            version: 1,
            created_at: new Date().toISOString()
        } as any);
        writeStagingStore(store);
        alert("Программа успешно сгенерирована! (Демо-режим)");
        await reloadPlans();
        setBusy(false);
        return;
      }

      const userId = terminalUserId || "terminal-unknown";
      const appId = commercial ? "ida" : "parent_navigator";
      const apiUrl = localStorage.getItem("API_ENDPOINT") || "https://api.prevention.school/api";
      
      const res = await fetch(`${apiUrl}/case/${registrySubjectId}/ipr`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json"
        },
        body: JSON.stringify({ userId, app_id: appId, plan_months: commercial ? 3 : 6, terminal_request: 1 })
      });
      const data = await res.json();
      if (data.ok) {
        alert("Программа успешно сгенерирована!");
        await reloadPlans();
      } else {
      ...
      {!registryEnabled ? (
        ...
        <>
          ...
          <section className="card ipr-child-form">
            <h3>{commercial ? "ИПР клиента" : "ИПР обучающегося"}</h3>
            <RegistrySubjectFioField
              ...
              label="ФИО"
              hint={commercial ? "Введите фамилию и выберите клиента из списка — его ИПР откроется ниже." : "Введите фамилию и выберите обучающегося из реестра — его ИПР откроется ниже."}
              showGradeClass={!commercial}
            />
            {registrySubjects.length === 0 && (
              <p className="muted tiny">
                {commercial ? "Список клиентов пуст. " : "Реестр пуст. "}
                <button type="button" className="linkish" onClick={onOpenRegistry}>
                  {commercial ? "Добавить клиента" : "Добавить обучающегося"}
                </button>
              </p>
            )}
            {registrySubjects.length > 0 && (
          ...
          <section className="card workspace-journal-card">
            <div className="workspace-journal-head">
              ...
              <div className="group-session-editor-actions">
                ...
                <button type="button" className="ob-btn primary" onClick={generateIprFromConsultations}>
                  Сгенерировать по консультациям
                </button>
              </div>
            </div>
