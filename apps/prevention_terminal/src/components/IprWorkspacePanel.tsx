    ...
    <div className="workspace-panel-stack ipr-workspace">
      {!registrySubjectId && (
        <section className="card">
          <h2>IEP — Individualized Education Program</h2>
          <p className="muted">
            An IEP is maintained only for records in the <strong>Registry</strong> (PII and personal file data). {PLAN_JOURNAL_INTRO}
          </p>
        </section>
      )}
      ...
      {!registryEnabled ? (
        ...
        <>
          ...
          <section className="card ipr-child-form">
            <h3>IEP of Accompanied Student</h3>
            <RegistrySubjectFioField
              ...
              label="Full Name"
              hint="Enter surname and select a student from the registry — their IEP will open below."
          ...
          <section className="card workspace-journal-card">
            <div className="workspace-journal-head">
              <div>
                <h3>Plan Journal</h3>
                <p className="muted tiny">{subjectLabel || "Registry record"}</p>
              </div>
            ...
            {busy && iprs.length === 0 ? <p className="muted">Loading…</p> : null}
            {!busy && iprs.length === 0 ? (
              <p className="muted">No IEP plans yet — click "New Plan".</p>
            ) : null}
            {iprs.length > 0 && (
              <WorkspaceListSortBar
                options={PERSON_CARD_SORT_OPTIONS}
