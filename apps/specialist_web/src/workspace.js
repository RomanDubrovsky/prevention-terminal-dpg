(function () {
    ...
    function init() {
        ...
        if (C.shouldSkipOnboarding() || C.onboardingOk() || isDemo) {
            showScreen('workspace');
            if (isDemo && !state.history.length) {
                const welcomeMsg = 'Приветствую! Я - демо-версия помощника. Вы можете попробовать задать мне вопросы, связанные с профилактикой и работой с подростками.';
                appendMsg('ai', welcomeMsg);
                state.history.push({ role: 'assistant', text: welcomeMsg });
            }
        } else {
